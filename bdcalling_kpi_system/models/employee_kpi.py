...
class EmployeeKpi(models.Model):
    ...
    minimum_target = fields.Float(
        related="grade_id.minimum_target", string="Minimum Target", readonly=True
    )
    ...
    employee_barcode = fields.Char(
        string="Employee Barcode", related="employee_id.barcode", readonly=True
    )

    '''# field for sales data -----*********************************
    total_sales_count = fields.Integer(
        "Total Sales Count", compute="_compute_sales_count"
    )
    total_sales_history = fields.One2many(
        "sale.order",
        "user_id",
        string="Total Sales History",
        compute="_compute_sales_history",
    )
    canceled_sales_history = fields.One2many(
        "sale.order",
        "user_id",
        string="Canceled Sales History",
        compute="_compute_sales_history",
    )
    # -----------------

    @api.depends("total_sales_history")
    def _compute_sales_count(self):
        for record in self:
            record.total_sales_count = len(record.total_sales_history)
    ...
    def _compute_sales_history(self):
        ...
        # Get all sales orders related to this employee in the current month
        sales_orders = self.env["sale.order"].search(
            [
                ...
                ),  # Match the employee's user_id (salesperson)
                (
                    "date_order",
                    ">=",
                    start_date,
                ),  # Sales orders from the start of the current month
                (
                    "date_order",
                    "<=",
                    end_date,
                ),  # Sales orders until the end of the current month
                (
                    "state",
                    "in",
                    ["sale", "cancel", "done"],
                ),  # Sales, canceled, or delivered (done) orders
            ]
        )  

        # Separate confirmed sales and canceled sales
        total_sales = sales_orders.filtered(lambda order: order.state == "sale")
        canceled_sales = sales_orders.filtered(lambda order: order.state == "cancel")

        # Set the One2many fields for sales history
        record.total_sales_history = total_sales
        record.canceled_sales_history = canceled_sales
'''
    # --------------------
    ...
    def action_calculate_record(self):
        ...
        if self.state == "paid":
            raise UserError(
                _("Cannot recalculate a record that has already been paid.")
            )
        print("RECORD", self)
        total_sales = self._get_employee_sales(
            self.employee_id, self.period_start, self.period_end
        )
        minimum_target = self.grade_id.minimum_target
        print("TOTAL SALES", total_sales, "MINIMUM TARGET", minimum_target)
        ...
        return {
            ...
            "params": {
                ...
                "message": _("KPI record for %s has been recalculated.")
    ...
    def action_bulk_calculate(self):
        ...
        if skipped_count > 0:
            message += (
                _(" %s records were skipped because they were already paid.")
                % skipped_count
            )
    ...
    def calculate_monthly_bonus(self):
        ...
        first_day_prev_month = date(today.year, today.month, 1) - relativedelta(
            months=1
        )
        ...
        for employee in employees:
            ...
            total_sales = self._get_employee_sales(
                employee, first_day_prev_month, last_day_prev_month
            )
            ...
            if is_penalty:
                shortfall_amount = minimum_target - total_sales
                _logger.info(
                    "Employee %s has shortfall of %s (minimum target: %s)",
                    employee.name,
                    shortfall_amount,
                    minimum_target,
                )
            ...
            if is_penalty:
                bonus_amount = -shortfall_amount
                _logger.info(
                    "Employee %s penalty amount: %s", employee.name, bonus_amount
                )
            else:
                _logger.info(
                    "Employee %s bonus amount: %s", employee.name, bonus_amount
                )
            ...
            if existing_record:
                ...
                if should_update and existing_record.state != "paid":
                    _logger.info(
                        "Updating existing KPI record for employee %s. Old sales: %s, New sales: %s",
                        employee.name,
                        existing_record.total_sales,
                        total_sales,
                    )
                    try:
                        existing_record.write(
                            {
                                ...
                                "state": (
                                    "draft"
                                    if existing_record.state == "confirmed"
                                    else existing_record.state
                                ),
                            }
                        )
                        _logger.info(
                            "KPI record updated for employee %s", employee.name
                        )
                    except Exception as e:
                        _logger.error(
                            "Error updating KPI record for employee %s: %s",
                            employee.name,
                            e,
                        )
                else:
                    _logger.info(
                        "No update needed for employee %s KPI record", employee.name
                    )
            else:
                ...
                try:
                    ...
                    _logger.info(
                        "New KPI record created for employee %s", employee.name
                    )
                except Exception as e:
                    _logger.error(
                        "Error creating KPI record for employee %s: %s",
                        employee.name,
                        e,
                    )
    ...
    def _get_employee_sales(self, employee, start_date, end_date):
        ...
        try:
            ...
            sales_total = sum(sales_orders.mapped('amount_total'))
            ...
            operations_total = sum(operations.mapped('monetary_value'))
            total = sales_total + operations_total         
            _logger.info('Employee %s: sales_total=%s, operations_total=%s, total=%s', employee.name, sales_total, operations_total, total)
            return total
    ...
    def _get_employee_sales(self, employee, start_date, end_date):
        ...
        try:
            ...
            _logger.info(
                "Employee %s sales orders: %s", employee.name, len(sales_orders)
            )
            if sales_orders:
                print("SALES ORDERS AMOUNT", sales_orders.mapped("delivery_amount"))
                return sum(sales_orders.mapped("delivery_amount"))
            else:
                print("NO SALES ORDERS FOUND FOR EMPLOYEE", employee.name)
                operations = (
                    self.env["project.operation"]
                    .sudo()
                    .search(
                        [
                ...
                _logger.info(
                    "Employee %s operations: %s", employee.name, len(operations)
                )
                return sum(operations.mapped("monetary_value"))
        except Exception as e:
            _logger.error(
                "Error calculating sales for employee %s: %s", employee.name, e
            )
            return 0.0
    ...
    def _calculate_bonus(self, employee, total_sales):
        ...
        try:
            if total_sales < employee.grade_id.minimum_target:
                return 0.0
            _logger.info(
                f"to find level---------------{employee.grade_id.name}----{total_sales}---{employee.company_id.name}"
            )
            ...
            if level:
                _logger.info(
                    "Employee %s matched level %s with bonus %s",
                    employee.name,
                    level.name,
                    level.bonus_amount,
                )
                ...
                _logger.info(
                    "No matching level found for employee %s with sales %s",
                    employee.name,
                    total_sales,
                )
                return 0.0
            ...
            _logger.error(
                "Error calculating bonus for employee %s: %s", employee.name, e
            )
            return 0.0
    ...
    def create_kpi_record_from_sale(self, employee, amount, date_completed):
        ...
        if not employee or not employee.role_id or not employee.grade_id:
            _logger.warning(
                "Cannot create KPI record: Employee %s has no role or grade assigned",
        ...
        if existing_record:
            if existing_record.state != "paid":
                _logger.info(
                    "Updating existing KPI record for employee %s. New total sales: %s",
                    employee.name,
                    total_sales,
                )
                existing_record.write(
            ...
            _logger.info(
                "Creating new KPI record for employee %s with sales: %s",
                employee.name,
                total_sales,
            )
            self.create(
