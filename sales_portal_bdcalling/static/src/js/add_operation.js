...
publicWidget.registry.CreateOperation = publicWidget.Widget.extend({
    ...
    async onOrderChange(ev) {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.$("#project").val(result.profile_name)
                // this.$("#profile_id").val(result.profile_id)
                // console.log("partner id:", result.partner_id.name)
                console.log("profile_name",result.profile_name)
    ...
    async onSubmit(ev) {
        ...
        if (!this.$('#team_id').val()) {
            missingFields.push("Please fill up Assign Team");
        }
    ...
    async onDelete(ev) {
        ...
        const orderId = this.$("#order_id").val();
        if (!orderId) {
            new Dialog(this, {
                title: "Error",
                size: 'medium',
                buttons: [{ text: 'Ok', close: true }],
                $content: $('<p/>', { text: "Order ID is missing. Cannot proceed with deletion." }),
            }).open();
            return;
        }
        ...
        new Dialog(this, {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: async () => {
                        try {
                            const response = await this.rpc('/delete/operation/order', { order_id: orderId });
                            ...
                            if (response.success) {
                                new Dialog(this, {
                                    ...
                                    buttons: [{
                                        ...
                                        close: true,
                                        click: function () {
                                            window.location.href = '/operation_dashboard';
                                        }
                                    }],
                                    $content: $(`
                                        <div class="text-center p-3">
                                            <div class="mb-3">
                                                <i class="fas fa-trash-alt" style="font-size: 48px; color: #dc3545;"></i>
                                            </div>
                                            <h4 class="text-danger mb-2">Deleted!</h4>
                                            <p class="text-muted">${response.message}</p>
                                        </div>
                                    `),
                                }).open();
                                ...
                                new Dialog(this, {
                                    ...
                                    size: 'medium',
                                    buttons: [{ text: 'Ok', close: true }],
                                    $content: $('<p/>', { text: response.message || "Deletion failed." }),
                                }).open();
                            }
                        } catch (err) {
                            ...
                            new Dialog(this, {
                                ...
                                size: 'medium',
                                buttons: [{ text: 'Ok', close: true }],
                                $content: $('<p/>', { text: "Something went wrong while deleting. Please try again." }),
                            }).open();
                        }
                    }
                }
            ...
            $content: $('<p/>', {
                text: `Are you sure you want to permanently delete Order ID: ${orderId}? This action cannot be undone.`
            }),
        }).open();
    },
